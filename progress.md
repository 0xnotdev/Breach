...
### 2026-08-15 19:18 IST — fix 33 published / fix 34 atomic lifecycle and failure finalization RED/GREEN

- Published fix 33 as `a4c11da5eab9b11b191d447c2e6c143c32574cad` (`fix 33: own integration web process`). Local, remote-tracking, and advertised `main` matched; the worktree was clean.
- GitHub Actions run `31887359781` passed lint, typecheck, coverage, build, and both real PostgreSQL migration invocations. Its controlled integration then failed immediately and safely because npm installs Vinext beneath `apps/web/node_modules`, while the new direct launch path referenced root `node_modules`. The production web image had the same latent entrypoint defect. Both now target the workspace-local CLI.
- RED: 8 focused failures established missing bounded lifecycle reason contracts, immutable migration 005, atomic scan claim/finalization, failure coverage persistence, snapshot cleanup after a mid-blob exception, and exact terminal/recovery graph evidence. A concurrent claim regression initially returned two winners under the in-memory PostgreSQL adapter, exposing the need for an in-process keyed admission lock in addition to PostgreSQL row locking and the unique `(repo_id, head_sha)` constraint.
- GREEN: a scan claim now atomically records the first commit time/SHA, advances logical `READY -> SCANNING` events, and admits exactly one same-process caller; PostgreSQL locks and uniqueness protect separate processes. Completion atomically finalizes both scan and candidate with bounded coverage and sanitized reasons. The public graph documents recoverable `RATE_LIMITED -> WAITING_FOR_COMMIT` and the five terminal states.
...
### 2026-08-15 19:35 IST — fix 36 published and CI GREEN / operator release path RED/GREEN

- Published fix 36 as `ccd59cc9e7f0791a2d85345af1d10e2591e13750`; local, remote-tracking, and advertised `main` matched. GitHub Actions run `31888578167` passed the new PostgreSQL metric joins and all prior gates.
- RED: operator-contract tests proved the README lacked all mandated task-oriented sections and CI had no fresh-volume Compose boot. The existing canary command also had no safe container path: the production worker intentionally excluded its fixture while PostgreSQL was not published to host-side tooling.
- GREEN: the README now gives exact first-run, health, canary, testing, reset, rate-limit, coverage, limitation, and no-verification instructions. Compose binds UI/health ports to loopback, uses worker `/readyz` for container health, hardens migration/proxy resource limits, and adds a separate metadata-only one-shot canary image without placing the fixture in the production worker.
- CI now destroys prior Compose state, boots PostgreSQL/migrations/API/proxy/web from a fresh volume without any real GitHub secret, verifies API and web health, runs the real runtime zero-retention canary, and always destroys the test volume. Focused operational/security tests, lint, strict typecheck, Compose config, and patch hygiene pass; full verification and GitHub Docker execution remain publication gates.
- Full local GREEN: `npm run verify` passed 15 files/136 tests, 98.25% statements/lines, 90.08% branches, 92.65% functions, all builds, 13 rendered-web checks, 5 Chromium journeys, zero high/critical production dependency vulnerabilities, and the 118-file tracked-secret audit. Docker Compose configuration also validates locally; this workstation has no usable Docker daemon, so the new build/start proof is delegated to the mandatory GitHub gate.

### 2026-08-15 19:41 IST — fix 37 GitHub RED / readiness and truthful-status RED/GREEN

- GitHub Actions run `31889010762` passed lint, typecheck, 136 tests/coverage, all builds, real PostgreSQL migrations, controlled full-stack, web/browser/audits, Compose config, and the four critical production image builds. Its new fresh-volume gate correctly failed because the web runtime changed to a workspace-local Vinext path while retaining `WORKDIR /opt/breach/apps/web`; the relative entrypoint therefore resolved beneath a duplicated `apps/web/apps/web` path and the container could not become healthy.
- Corrected the production web entrypoint to the absolute workspace-local Vinext CLI and made the always-run CI cleanup print bounded operator-service logs before destroying the volume, so future Docker startup failures remain diagnosable without source/GitHub payload logging.
- RED: worker readiness remained green after an earlier successful cycle even if PostgreSQL was currently unavailable or a later cycle had fatally stalled. UI source also retained unused canned `Collector online` / `Last event 2s ago` defaults and labeled a one-time findings fetch as `LIVE DATA`.
- GREEN: `/readyz` now performs a time-bounded PostgreSQL query, reports only `ready`/`unavailable`, and rejects failed/stopped schedulers plus a running cycle beyond five minutes. Canned liveness defaults are impossible at the component type boundary, successful list fetches say `API DATA`, and true `CONNECTED` remains exclusive to the open SSE stream.
- Focused scheduler, operations, and rendered-web regressions pass with lint, strict typecheck, Compose config, and patch hygiene. Full verification and the repaired GitHub fresh-volume/canary run remain publication gates.
- Full local GREEN: `npm run verify` passed 15 files/137 tests, 98.20% statements/lines, 90.11% branches, 92.70% functions, all builds, 14 rendered-web checks, 5 Chromium journeys, dependency audit, and the 119-file tracked-secret audit.

### 2026-08-15 19:48 IST — fix 38 published / clean-clone GREEN / operator-network RED/GREEN

- Published fix 38 as `b23ede4134761623bfc44532e7ba3f191523bd95`; local, remote-tracking, and advertised `main` matched.
- An independent clone of published `main` installed 555 packages from the lockfile with `npm ci --ignore-scripts --no-audit --no-fund`, then passed the complete 15-file/137-test verification, coverage, builds, 14 web checks, 5 Chromium journeys, dependency audit, and 119-file secret audit. No local generated file or existing `node_modules` was required.
- GitHub Actions run `31889299603` proved the absolute Vinext entrypoint: PostgreSQL, all five migrations, API, proxy, and web reached internal health, with Vinext serving on `0.0.0.0:3000`. The gate then failed its localhost API curl because API/web were attached only to the `internal: true` metadata network, so Docker created no host bindings despite valid Compose port declarations.
