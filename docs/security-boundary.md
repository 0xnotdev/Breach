...
The application egress policy admits only HTTPS requests on port 443 to `api.github.com` and `api.osv.dev`, plus explicitly configured internal service hostnames. It rejects userinfo, redirects to unapproved hosts, GitHub repository web URLs, arbitrary HTTP(S), alternate ports, and non-network schemes. Locally, the worker has no external bridge: Node routes HTTP(S) through the dedicated CONNECT proxy on an internal control network, and only that proxy joins the ordinary external bridge. API joins metadata plus a separate operator bridge, while web joins only the operator bridge, so loopback-published API/UI ports remain reachable without placing the worker on an external bridge. The proxy accepts the same two exact TLS authorities and denies ordinary HTTP forwarding. Production must additionally apply the FQDN-aware default-deny policy in `deploy/network-policy.md`; an ordinary Docker bridge is not a hostname firewall.
