...
COPY apps ./apps
RUN npm ci --ignore-scripts --no-audit --no-fund && npm run build --workspace @breach/api
...
COPY --from=build --chown=breach:breach /opt/breach/apps/api/dist ./apps/api/dist
USER breach
EXPOSE 8080
